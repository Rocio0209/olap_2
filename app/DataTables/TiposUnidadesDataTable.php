...
class TiposUnidadesDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('Acciones', function($row){
                ...
                if($user->can('updt_'.$this->permiso)){
                    $acciones.=' <a id-accion1="'.$row->idtipo_unidad.'" class="modificar"><span data-bs-toggle="tooltip" class="fa fa-edit" title="Modificar"> </span></a> ';
                }
                if($user->can('del_'.$this->permiso)){
                    $acciones.= ' <a id-accion1="'.$row->idtipo_unidad.'" class="eliminar"><span data-bs-toggle="tooltip" class="fa fa-trash" title="Eliminar"> </span></a> ';
                }
                return $acciones;
            })
            ->rawColumns(['Acciones'])
            ->setRowId('idtipo_unidad');
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(TipoUnidad $model): QueryBuilder
    {
    ...
    {
        return [
            Column::make('idtipo_unidad')->title('ID'),
            Column::make('tipo_unidad')->title('Tipo Unidad'),
    ...
    {
        return 'TiposUnidad_' . date('YmdHis');
    }
}
