...
class TiposAdministracionDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('Acciones', function($row){
                ...
                if($user->can('updt_'.$this->permiso)){
                    $acciones.=' <a id-accion1="'.$row->idtipo_administracion.'" class="modificar"><span data-bs-toggle="tooltip" class="fa fa-edit" title="Modificar"> </span></a> ';
                }
                if($user->can('del_'.$this->permiso)){
                    $acciones.= ' <a id-accion1="'.$row->idtipo_administracion.'" class="eliminar"><span data-bs-toggle="tooltip" class="fa fa-trash" title="Eliminar"> </span></a> ';
                }
                return $acciones;
            })
            ->rawColumns(['Acciones'])
            ->setRowId('idtipo_administracion');
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(TipoAdministracion $model): QueryBuilder
    {
    ...
    {
        return [
            Column::make('idtipo_administracion')->title('ID'),
            Column::make('tipo_administracion')->title('Tipo Administracion'),
    ...
    {
        return 'TiposAdministracion_' . date('YmdHis');
    }
}
