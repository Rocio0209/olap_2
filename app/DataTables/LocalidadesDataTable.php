...
class LocalidadesDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('Acciones', function($row){
                ...
                if($user->can('updt_'.$this->permiso)){
                    $acciones.=' <a id-accion1="'.$row->idestado.'" id-accion2="'.$row->idmunicipio.'" id-accion3="'.$row->idlocalidad.'" class="modificar"><span data-bs-toggle="tooltip" class="fa fa-edit" title="Modificar"> </span></a> ';
                }
                if($user->can('del_'.$this->permiso)){
                    $acciones.= ' <a id-accion1="'.$row->idestado.'" id-accion2="'.$row->idmunicipio.'" id-accion3="'.$row->idlocalidad.'" class="eliminar"><span data-bs-toggle="tooltip" class="fa fa-trash" title="Eliminar"> </span></a> ';
                }
                return $acciones;
            })
            ...
            ->rawColumns(['Acciones'])
            ->setRowId('idlocalidad');
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(Localidad $model): QueryBuilder
    {
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('idlocalidad')->title('IDLocalidad'),
    ...
    {
        return 'Localidades_' . date('YmdHis');
    }
}
