...
class InstitucionesDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('Acciones', function($row){
                ...
                if($user->can('updt_'.$this->permiso)){
                    $acciones.=' <a id-accion1="'.$row->idinstitucion.'" class="modificar"><span data-bs-toggle="tooltip" class="fa fa-edit" title="Modificar"> </span></a> ';
                }
                if($user->can('del_'.$this->permiso)){
                    $acciones.= ' <a id-accion1="'.$row->idinstitucion.'" class="eliminar"><span data-bs-toggle="tooltip" class="fa fa-trash" title="Eliminar"> </span></a> ';
                }
                return $acciones;
            })
            ->rawColumns(['Acciones'])
            ->setRowId('idinstitucion');
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(Institucion $model): QueryBuilder
    {
    ...
    {
        return [
            Column::make('idinstitucion')->title('ID'),
            Column::make('institucion')->title('Institucion'),
            Column::make('descripcion_corta')->title('Descripción Corta'),
            Column::make('iniciales')->title('Iniciales'),
    ...
    {
        return 'Instituciones_' . date('YmdHis');
    }
}
