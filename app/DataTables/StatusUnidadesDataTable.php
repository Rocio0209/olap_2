...
class StatusUnidadesDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('Acciones', function($row){
                ...
                if($user->can('updt_'.$this->permiso)){
                    $acciones.=' <a id-accion1="'.$row->idstatus_unidad.'" class="modificar"><span data-bs-toggle="tooltip" class="fa fa-edit" title="Modificar"> </span></a> ';
                }
                if($user->can('del_'.$this->permiso)){
                    $acciones.= ' <a id-accion1="'.$row->idstatus_unidad.'" class="eliminar"><span data-bs-toggle="tooltip" class="fa fa-trash" title="Eliminar"> </span></a> ';
                }
                return $acciones;
            })
            ->rawColumns(['Acciones'])
            ->setRowId('idstatus_unidad');
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(StatusUnidad $model): QueryBuilder
    {
    ...
    {
        return [
            Column::make('idstatus_unidad')->title('ID'),
            Column::make('status_unidad')->title('status_unidad'),
    ...
    {
        return 'StatusUnidades_' . date('YmdHis');
    }
}
