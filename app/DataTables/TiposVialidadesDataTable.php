...
class TiposVialidadesDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('Acciones', function($row){
                ...
                if($user->can('updt_'.$this->permiso)){
                    $acciones.=' <a id-accion1="'.$row->idtipo_vialidad.'" class="modificar"><span data-bs-toggle="tooltip" class="fa fa-edit" title="Modificar"> </span></a> ';
                }
                if($user->can('del_'.$this->permiso)){
                    $acciones.= ' <a id-accion1="'.$row->idtipo_vialidad.'" class="eliminar"><span data-bs-toggle="tooltip" class="fa fa-trash" title="Eliminar"> </span></a> ';
                }
                return $acciones;
            })
            ->rawColumns(['Acciones'])
            ->setRowId('idtipo_vialidad');
    }

    /**
     * Get the query source of dataTable.
     */
    public function query(TipoVialidad $model): QueryBuilder
    {
    ...
    {
        return [
            Column::make('idtipo_vialidad')->title('ID'),
            Column::make('tipo_vialidad')->title('Tipo Vialidad'),
    ...
    {
        return 'TiposVialidades_' . date('YmdHis');
    }
}
