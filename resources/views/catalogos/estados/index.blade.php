<x-app-layout>
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('estados-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('estados-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement id="idestado" label="IDEstado" required=true tipo="number" ></x-formElement>
            <x-formElement id="estado" label="Estado" required=true ></x-formElement>
            <x-formElement id="siglas" label="Siglas" required=true ></x-formElement>
        </x-modal-form>
