<x-app-layout>
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('tipos_vialidades-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('tipos_vialidades-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('tipos_vialidades-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement anchocampo="8" ancholabel="4" id="idtipo_vialidad" label="IDTipo Vialidad" required=true tipo="number" ></x-formElement>
            <x-formElement anchocampo="8" ancholabel="4" id="tipo_vialidad" label="Tipo Vialidad" required=true ></x-formElement>
        </x-modal-form>
