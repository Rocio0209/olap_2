<x-app-layout>
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('status_unidades-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('status_unidades-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('status_unidades-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement anchocampo="8" ancholabel="4" id="idstatus" name="idstatus_unidad" label="IDEstatus Unidad" required=true tipo="number" ></x-formElement>
            <x-formElement anchocampo="8" ancholabel="4" id="status_unidad" label="Estatus Unidad" required=true ></x-formElement>
        </x-modal-form>
