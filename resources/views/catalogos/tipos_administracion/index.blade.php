<x-app-layout>
    ...
    @push('scripts')
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                //Variables Globales
                const elemento='Tipo Administración';
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('tipos_administracion-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('tipos_administracion-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('tipos_administracion-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement id="idtipo_administracion" label="ID" required=true tipo="number"></x-formElement>
            <x-formElement ancholabel="4" anchocampo="8" id="tipo_administracion" label="Tipo Administracion" required=true ></x-formElement>
        </x-modal-form>
