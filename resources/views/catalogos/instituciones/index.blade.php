<x-app-layout>
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('instituciones-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('instituciones-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('instituciones-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement anchocampo="8" ancholabel="4"  id="idinstitucion" label="IDInstitución" required=true tipo="number" ></x-formElement>
            <x-formElement anchocampo="8" ancholabel="4"  id="institucion" label="Institución" required=true ></x-formElement>
            <x-formElement anchocampo="8" ancholabel="4"  id="descripcion_corta" label="Descripción Corta" required=true ></x-formElement>
            <x-formElement anchocampo="8" ancholabel="4"  id="iniciales" label="Iniciales" required=true ></x-formElement>
        </x-modal-form>
