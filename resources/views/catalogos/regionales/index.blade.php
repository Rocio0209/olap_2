<x-app-layout>
    ...
    @push('scripts')
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                //Variables Globales
                const elemento='Coordinación Regional';
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('regionales-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('regionales.index') }}"+"/"+ids.id1 +'/'+ ids.id2,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('regionales.index') }}" + "/" + ids.id1 + '/' + ids.id2 + '/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement id="idestado" label="Estado" required=true tipo="select" classcampo="select2" >
                @foreach( $estados as $estado )
                <option value="{{ $estado->idestado }}" >{{ $estado->estado }}</option>
                    @endforeach
            </x-formElement>
            <x-formElement id="idregional" label="IDRegional" required=true tipo="number"></x-formElement>
            <x-formElement id="regional" label="Regional" required=true ></x-formElement>
        </x-modal-form>
