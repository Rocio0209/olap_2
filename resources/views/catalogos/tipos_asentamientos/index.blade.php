<x-app-layout>
    ...
    @push('scripts')
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                //Variables Globales
                const elemento='Tipo Asentamiento';
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('tipos_asentamientos-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('tipos_asentamientos-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('tipos_asentamientos-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement anchocampo="8" ancholabel="4" id="idtipo_asentamiento" label="IDTipo Asentamiento" required=true tipo="number" ></x-formElement>
            <x-formElement anchocampo="8" ancholabel="4" id="tipo_asentamiento" label="Tipo Asentamiento" required=true ></x-formElement>
        </x-modal-form>
