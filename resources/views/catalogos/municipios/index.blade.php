<x-app-layout>
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function () {
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if (window.valida('#' + idform)) {
                        $.ajax({
                            ...
                            url: "{{ route('municipios-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function () {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('municipios-api.store') }}" + "/" + ids.id1 + '_' + ids.id2,
                        headers: {
                ...
                $('#dataTable-table').on('click', 'a.modificar', function () {
                    ...
                    $.get("{{ route('municipios-api.index') }}" + '/' + + ids.id1 + '_' + ids.id2 + '/edit', function (data) {
                    ...
                    switch (id) {
                        case 'idestado':
                            if (value !== "") {
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        for (let i in data.lista) {
                                            opciones += '<option value="' + data.lista[i].idregional + '">' + data.lista[i].regional + '</option>';
                                        }
                                        $('#idregional').html(opciones);
                                    }, error: function (data) {
                                        window.hidecarga();
                                ...
                                $('#idregional').html("<option value=''>Seleccione...</option>");
                            }
                            break;
                    }
                });
    ...
    @push('modals')
        ...
        <x-modal-form>
            ...
            <x-formElement id="idregional" label="Regional" required=true tipo="select"></x-formElement>
            <x-formElement id="idmunicipio" label="IDMunicipio" required=true tipo="number"></x-formElement>
            <x-formElement id="municipio" label="Municipio" required=true></x-formElement>
        </x-modal-form>

        <!-- Modal Para mostrar confirmación de eliminación-->
        <x-modal-aviso id="eliminaModal" accionBtnClass="btn-danger" closeDataModalClass="btn-primary" modaltype="danger"
            textTitle="¡Atención!" accionBtnTxt="Eliminar" closeDataModalTxt="Cancelar">
