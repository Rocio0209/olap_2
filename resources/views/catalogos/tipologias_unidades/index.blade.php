<x-app-layout>
    ...
    @push('scripts')
        ...
        <script type="module" nonce="{{ csp_nonce() }}">
            $(document).ready(function(){
                //Variables Globales
                const elemento='Tipología Unidad';
                ...
                $('#accionBtnmodalData').click(function (e) {
                    ...
                    if(window.valida('#'+idform)){
                        $.ajax({
                            ...
                            url: "{{ route('tipologias_unidades-api.store') }}",
                            type: "POST",
                            dataType: 'json',
                ...
                $('#avisoActionBtneliminaModal').click(function(){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('tipologias_unidades-api.store') }}"+"/"+ids.id1,
                        headers:{
                ...
                $('#dataTable-table').on('click','a.modificar', function () {
                    ...
                    $.get("{{ route('tipologias_unidades-api.index') }}" +'/' + ids.id1 +'/edit', function (data) {
    ...
    @push('modals')
        ...
        <x-modal-form>
            @csrf
            <x-formElement ancholabel="4" anchocampo="8" id="tipologia_unidad" label="Tipología Unidad" required=true ></x-formElement>
            <x-formElement ancholabel="4" anchocampo="8" id="clave_tipologia" label="Clave Tipología" required=true ></x-formElement>
        </x-modal-form>
